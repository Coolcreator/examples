...
parser.add_argument('--max', type=int, choices=[15, 20, 25],
                    default=15, help='max points number')
...
tests = [
    ['mkdir testdir', 'cd testdir', 'pwd | tail -c 8',
     '   pwd | tail -c 8'],
    [
...
" | grep 4''',
    ...
    ['false && echo 123', 'true && echo 123',
     'true || false && echo 123', 'true || false || true && echo 123',
     'false || echo 123'],
    ["sleep 0.5 && echo 'back sleep is done' &",
     "echo 'next sleep is done'", 'sleep 0.5'],
    ]
...
for i in range(line_count):
    ...
    if output[i] != etalon[i]:
        print('Error in line {}. Expected:\n{}'.format(i + 1, etalon[i]))
        is_error = True
        break
if not is_error and etalon_len != output_len:
    print('Different line count. Got {}, expected {}'.format(output_len,
            etalon_len))
    is_error = True
p.terminate()
if is_error:
